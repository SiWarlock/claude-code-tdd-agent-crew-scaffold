...
Everything above describes the **Claude Code** host. The generator can also target **Codex CLI** (chosen first, at §4.0; recorded as `host` in the manifest). On Codex the **solo core** is the supported path — root + nested `AGENTS.md`, `.agents/skills/<name>/SKILL.md` slash commands (never bare-root `skills/`, which Codex's real skill loader doesn't scan), `.codex/config.toml` (`[mcp_servers]` + `[[hooks.PreToolUse]]` guards — the canonical project-config location) — and it maps directly onto the single-operator model above (one Codex session running `/tdd`). The whole agent-team coordination layer (team lead, `SendMessage`, shared task list, statusline auto-cycle, per-track worktrees) has **no 1:1 Codex equivalent** and is dropped from the Codex solo core.

> ⚠️ **EXPERIMENTAL — Codex multi-agent team overlay (WIP).** An opt-in overlay re-expresses the team layer on Codex's **unstable** `collaboration_mode` / `spawn_agent` v2 APIs (root-session-as-lead → orchestrator → per-slice implementer → reviewers; a filesystem ledger replaces the shared task list; per-slice teardown replaces the context auto-cycle; a 3-failure circuit breaker + a `codex-team-preflight` round-trip guard it, falling back to solo on any failure). It carries hard caveats — no native worktree isolation, `codex exec` exits 0 on failure, `--output-schema` support by model family is version-dependent (confirm on yours), no context-% signal — and is **OFF by default** behind two switches (a generation-time opt-in **and** Codex's runtime collab mode). Treat it as WIP; pin a known-good Codex version. Full design + caveats: **`docs/codex/team-overlay.md`**.
...
└── archive/ …                 # Round-history log (IMPLEMENTATION_LOG.md) + materialized phase-exit checklists + archived plans
...
| **`/phase-exit <phase>`** | Orchestrator | **Executes** the tracker's phase-exit checklist as a row→executor mapper (hardcodes no rows): auditor fan-outs (`reachability-auditor`, `arch-drift-auditor`, policy-resolved security review) in one parallel message, `spec-lint tests` + posture-gated rows via Bash, per-row ticks recorded as they pass, full reports → `docs/audits/`, CLEAR/BLOCKED verdict to `docs/archive/IMPLEMENTATION_LOG.md`. Dispatch at the START of a round. |
...
The single source of truth for "what's done, what's next." Section order: phase note · session protocol · reference deadlines · Currently in progress · Carry-forward (triaged every `/orchestrate-end`) · **Owner gates & arming ledgers** · deliverable map · **Parallelization plan (Track map)** · phase exit checklist (template — materialized per-phase copies live in `docs/archive/`) · phase sections with spec anchors, a per-phase `Track:` / `Depends on (phases):` / one-line `**Gate:**` pointer, and dense checkbox tasks (each with a `Depends on:` edge) · an **optional Demo phase** · Trims/Nice-to-Haves Catalog · Decisions tabled · Log (a pointer stub → `docs/archive/IMPLEMENTATION_LOG.md`). **Living sections are bounded, not append-only** — Carry-forward caps at ~7 items (force-triaged), Currently-in-progress is REPLACED each round (≤3 items / ≤15 lines), and the plan carries **no inline Log** — round history lives append-only in `docs/archive/IMPLEMENTATION_LOG.md`, reached through the Log section's one-line pointer stub — so the sectioned read stays cheap late in the project. The plan is **sized by the build posture** (from `{{ARCH_DOC}}`): production-grade promotes hardening early; MVP/prototype stays lean. The **Parallelization plan** is the authority for the project's parallel tracks → worktrees → team names (team mode); a demo is an explicit *optional* phase, never in the mandatory spine.
...
A fourth, **warn-only** hook — `scripts/hooks/team-event-log.sh`, wired under the agent-team `TeammateIdle` + `TaskCompleted` events — appends an idle/completion timeline to `~/.claude/team-events/<label>.jsonl` (no-op outside team sessions). It is purely observational: it **always exits 0** and never blocks. An exit-2 `TeammateIdle` ("keep working") is deliberately avoided because it would break the Step-2.5 / Step-9 idle-for-review pattern (an implementer idling while it waits for `APPROVED.` is correct) and slice atomicity. The log lets the lead spot the documented drift "a teammate idled but never marked its in-progress task completed" — reconciliation, not enforcement.
...
- **Run it from inside your project**: `/scaffold-upgrade [--check] [--from <sha>] [--to <ref>] [--scaffold <path>] [--auto]`. The skill is global (**not vendored into the project**, so the upgrade logic never goes stale) and **obtains the template source itself** — by default a **full** clone of the manifest's `scaffoldingRepo` at the target ref into a throwaway dir; pass `--scaffold <path>` to use a local checkout instead (offline, or a dev working copy). You never need this repo pre-cloned at a known path — the manifest records where the templates came from. `--check` reports drift ("N commits / M files behind") without writing — safe in CI.
...
10. **The hooks enforce mechanics, not judgment.** territory/git/secrets guards stop the wrong tool call; they can't catch a wrong decision expressed through an allowed one. The checkpoints (Step 2.5, Step 9, the gates) remain the judgment layer. The `TeammateIdle`/`TaskCompleted` event hook is **warn-only by design** — it logs, it never blocks idle/completion, because an exit-2 idle-block would fight the idle-for-review pattern; catching "idled without marking the task done" is left to lead reconciliation off the event log, not auto-enforcement.
