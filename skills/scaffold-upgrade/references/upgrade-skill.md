...
| **`accreted`** | `area-LESSONS.md` (empty-by-design, accretes via Step-9), `IMPLEMENTATION_PLAN.md` living sections (Currently-in-progress, Carry-forward, Decisions tabled, phase checkboxes; the `Log` is no longer a living section — a fixed pointer stub, with round history in `docs/archive/IMPLEMENTATION_LOG.md`), area-`CLAUDE.md` lookup + cross-doc-invariants tables. | **LEAVE ALONE.** Body is never touched. Only the **skeleton/format** regions (header blockquote, the lesson-format code block, fixed section *headings*) are merge candidates, and only as a **PROPOSE** suggestion — never auto-applied. | The model decides whether a skeleton change is cosmetic (skip) or a **structural format migration** that the accreted body must conform to → routes it to the **migration registry** (§6), which is the *only* path that may rewrite accreted content, and only via an explicit human-gated, idempotent migration. |
