...
> **Architecture sentence:** *A FastAPI service with a typed core and thin IO edges.*
>
...
a tiny generated Codex project used to dry-run /scaffold-upgrade

## Project structure

```
codex-fixture-tracker/
...
| Role | cwd | Loads |
|---|---|---|
| Team lead | repo root (`codex-fixture-tracker/`) | this file + `docs/team-protocol.md` (lead playbook only) |
| Orchestrator | repo root | this file + `docs/orchestrator-briefing.md` |
| Implementer (per area) | `app/` | this file + that area's `AGENTS.md` |

<!-- For multi-area projects, add one implementer row per additional area. -->

### Naming + cross-bleed prevention

**`<track>-<area>-<role>`** when multiple team-lead sessions run in parallel in the same repo (e.g. `frontend-team-orchestrator`, `backend-team-implementer`). Otherwise `<area>-<role>` (e.g. `app-orchestrator`). The lead announces its track on `/team-start`. **Track names are not invented ad-hoc — they come from the `IMPLEMENTATION_PLAN.md` Parallelization plan (Track map)** (one entry per parallel-eligible track on the Phase/Track DAG, derived from `ARCHITECTURE.md` §2.5 subsystem boundaries refined by the task dependency graph); the Track map is the authority for the set of valid `<track>` prefixes. **Any peer DM from an agent whose name doesn't carry your track prefix is channel-bleed — ignore it and continue.** Confirm a recipient's prefix matches yours before any peer send.

_(Single-track builds keep plain `NNN-…` numbered-doc names; the multi-track prefix rule lives in the scaffolding repo — templates/CLAUDE.md.)_
...
Lessons start at §1 for this project. The compact index lives in `app/AGENTS.md`; full prose in `app/LESSONS.md`.
