<!--
  TEMPLATE: IMPLEMENTATION_PLAN.md → write to repo root.
...
# IMPLEMENTATION_PLAN.md — CodexFixtureTracker
...
> **Team mode only.** A *track* is a set of phases whose subsystems form a dependency-isolated region of the `ARCHITECTURE.md` §2.5 DAG. Tracks with no unsatisfied upstream-track dependency run **in parallel — each in its own git worktree with its own agent team**. A single-track plan deletes this section.
...
|---|---|---|---|---|
| <track-a> | <phase IDs> | <area dir(s)> | `../codex-fixture-tracker-<track-a>` (`track/<track-a>`) | `<track-a>-<area>-orchestrator` / `-implementer` |
| <track-b> | <phase IDs> | <area dir(s)> | `../codex-fixture-tracker-<track-b>` (`track/<track-b>`) | `<track-b>-<area>-orchestrator` / `-implementer` |
...
**Shared contracts across tracks** (freeze before tracks fork — the `ARCHITECTURE.md` Appendix A models crossing a §2.5 edge): <the models / files multiple tracks read>.
...
- [ ] **Perf budgets met, or the regression is flagged as a Finding** — run the phase's benchmark task(s) against the `ARCHITECTURE.md` budgets; phases with no stated budgets tick with `n/a — no budgets (deliberate deferral recorded)`. _(production-grade, when budgets exist)_
...
## Phase P0, P1 — <phase name>

**Goal:** <one-paragraph goal>.

**Spec anchors:** `ARCHITECTURE.md §X`, §Y.
...
<!-- OPTIONAL `Implements: REQ-x[, REQ-y]` line — add ONLY when one § maps to multiple REQs and this
     ...
     phase's `Spec anchors:` line + the ARCHITECTURE.md Spec Anchor Index already map REQ → § → task, and a
...
<!-- ▼ EXAMPLE BLOCK [id=optional-demo-phase]: OPTIONAL Demo phase — include ONLY if a demo / live walkthrough is an explicit deliverable. Check the **Build posture** in ARCHITECTURE.md: a production-grade build usually OMITS this phase (ship a deployable/operable slice instead); an MVP/prototype build makes it the natural near-final slice. Delete this whole block when no demo is in scope. ▼ -->
...
**Spec anchors:** `ARCHITECTURE.md §X` — the flows the demo exercises.
