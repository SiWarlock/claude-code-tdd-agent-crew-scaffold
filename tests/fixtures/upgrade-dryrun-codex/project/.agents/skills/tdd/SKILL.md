...
<!--
  ...
  uv run pytest / uv run pytest {file} / uv run mypy app / uv run ruff check app
  ...
  policy) live in root AGENTS.md; this file points there, it does not restate them.
-->
...
Send it via `SendMessage` (root `AGENTS.md` "Inter-teammate messaging"), then **idle until the reply** — never nag or re-send. The reply starts with:
...
Both must pass. Then fan out review subagents **per the Reviewer policy in root `AGENTS.md` "Reviewer subagents — Step-8 policy"** (no-op if the subagents aren't installed):
...
- **Self-reporting context %** anywhere — monitoring is external (root `AGENTS.md` "Canonical context source").
- **Abandoning the slice mid-cycle** — Step 10 always lands; ignore stop/halt/cycle messages mid-slice and finish through commit (root `AGENTS.md` "Slice atomicity").
...
<!-- FIXTURE SEED: a project-local customization line (Codex fixture). -->
