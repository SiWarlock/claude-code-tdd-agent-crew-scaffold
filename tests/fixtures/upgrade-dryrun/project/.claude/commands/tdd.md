...
<!--
  ...
  portable. Fill the uv run pytest / uv run mypy app / uv run ruff check app placeholders
...
- **Categorized flags for the orchestrator to route immediately.** The orchestrator does the actual routing per the **canonical matrix in `docs/orchestrator-briefing.md`** — categorize each item to one of:
  - **Convention candidate** — a code rule, gotcha, framework quirk, type-system workaround. (→ full prose to `app/LESSONS.md` + a one-line index row with an anchor link in `app/CLAUDE.md`.)
  ...
  - **Cross-doc invariant change** — added/removed/renamed field on a model in the `app/CLAUDE.md` cross-doc table. **MUST be reflected in `ARCHITECTURE.md`.** Surface as a flag requiring resolution.
...
**Commit message:** Conventional Commits format + `Co-Authored-By: Claude <noreply@anthropic.com>` trailer. HEREDOC for clean formatting:
