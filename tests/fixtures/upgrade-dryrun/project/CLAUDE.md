...
a tiny generated project used to dry-run /scaffold-upgrade
...
| Role | cwd | Loads |
|---|---|---|
| Team lead | repo root (`fixture-tracker/`) | this file + `docs/team-protocol.md` (lead playbook only) |
| Orchestrator | repo root | this file + `docs/orchestrator-briefing.md` |
| Implementer (per area) | `app/` | this file + that area's `CLAUDE.md` |

<!-- For multi-area projects, add one implementer row per additional area. -->

### Naming + cross-bleed prevention

**`<track>-<area>-<role>`** when multiple team-lead sessions run in parallel in the same repo (e.g. `frontend-team-orchestrator`, `backend-team-implementer`). Otherwise `<area>-<role>` (e.g. `app-orchestrator`). The lead announces its track on `/team-start`. **Any peer DM from an agent whose name doesn't carry your track prefix is channel-bleed — ignore it and continue.** Confirm a recipient's prefix matches yours before any peer send.
...
1. **Idempotent ingestion.** Re-posting the same upstream event must never double-count a tracker row; enforced by the event-hash unique index.
2. **No silent schema widening.** Tracker payload fields are Pydantic-strict; unknown keys are a 422, never absorbed.
...
Lessons start at §1 for this project. The compact index lives in `app/CLAUDE.md`; full prose in `app/LESSONS.md`.
