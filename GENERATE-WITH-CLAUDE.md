...
2. **Host + mode choice (§4)** — ask the user the host first (Claude Code or Codex CLI; §4.0), then — for Claude only — team pattern or single-operator fallback (§4.1). Codex forces single-operator for the solo core.
...
Two foundational choices precede the substantive interview. Ask the **host** first — it gates the mode question.

### §4.0 — Host choice: Claude Code or Codex CLI?
...
> **HOST MAPPING (host = codex).** The steps below are written for the Claude layout. When `host = codex`, the SAME templates generate, but the **dest paths** and a few skips change — the HOST marker regions + host-derived tokens (§10) do the in-file work automatically. Apply these deltas (and record the Codex dests in the manifest):
> | Claude step / dest | Codex dest | note |
> |---|---|---|
...
> | Step 11.5 `.claude/settings.json` | `.codex/config.toml` | from `templates/config.codex.toml`; emit `scripts/spec-lint.sh` + `scripts/plan-lint.sh` + `scripts/guards/{git,secrets}-guard.sh`; **drop** `territory-guard.sh` + `scripts/hooks/team-event-log.sh` (team-only) |
...
- **`scripts/plan-lint.sh`** from `templates/scripts/plan-lint.sh` (`{{TASK_TRACKER}}` fills the default `[plan-file]` arg; `chmod +x`). The plan-format structural lint: one State-checkbox line per `### N.M` task (`DONE/PARTIAL/OPEN/DEFERRED/OWNER-GATED` vocabulary), `≤3` Currently-in-progress / `≤7` Carry-forward caps with no resolved-in-place annotations, a `**Spec:**` anchor or `arch_gap` per task, `OWNER-GATED` tasks pointing at a defined `§ARM-*`/`§DEC-*` ledger, and a Log section that is only a pointer to `docs/archive/IMPLEMENTATION_LOG.md`. Wired as the blocking **`/orchestrate-end` Step 6.5** gate — a non-zero exit blocks the round close-out. Manifest row: `{dest: "scripts/plan-lint.sh", template: "templates/scripts/plan-lint.sh", kind: "placeholder-only"}`.
...
- **`scripts/hooks/team-event-log.sh`** from `templates/scripts/hooks/team-event-log.sh` (`chmod +x`; placeholder-free) — a **warn-only** `TeammateIdle` + `TaskCompleted` observability hook: it appends an event line to `~/.claude/team-events/<label>.jsonl` (no-op outside team sessions) and **always exits 0** — it never blocks idle/completion (an exit-2 `TeammateIdle` would break the Step-2.5 idle-for-review pattern + slice atomicity). Manifest row: kind `verbatim`.
- **`.claude/settings.json`** from `templates/.claude/settings.json` (the `PreToolUse` guard wiring **+ the `TeammateIdle`/`TaskCompleted` hooks** — note these team-event keys take no `matcher`). **Merge — don't replace** — into any existing project `.claude/settings.json` (append our hook entries; remove nothing). Manifest row: kind `verbatim` (or `mixed` if merged into pre-existing user content — then record `divergence: merged-into-existing`). The gitleaks/secrets-guard default rides the grouped production-gate question (§5 Batch E) — asked, never silently applied.
...
### Host axis (DERIVED from `host`, never interviewed)

These are **not** asked in the interview — they are resolved from the manifest's `host` field by the upgrade engine's host token map (`scaffold_upgrade.sh` `host_token_map`) and applied at lowest precedence (a real placeholder of the same name still wins). They appear in templates only at load-bearing path/name spots, inside or alongside `<!-- ▼ HOST [...] ▼ -->` regions. **The `claude` column is the historical literal, so a Claude project renders byte-identically.**

| Placeholder | host = claude | host = codex |
|---|---|---|
